...
export default function DashboardPage() {
  ...
  useEffect(() => { const storedUser = localStorage.getItem('nfse_user'); if (!storedUser) { router.replace('/login'); return; } setUser(JSON.parse(storedUser) as StoredUser); void loadCompanies('', { updateAll: true }); }, [router]);
  ...
  async function requestApi(path: string, options: RequestInit = {}) { const response = await fetch(`${apiBase}${path}`, { ...options, credentials: 'include', headers: { 'Content-Type': 'application/json', ...(options.headers || {}) } }); if (response.status === 401) { localStorage.removeItem('nfse_user'); router.replace('/login'); throw new Error('Sessão expirada.'); } const text = await response.text(); const data = text ? JSON.parse(text) : null; if (!response.ok) throw new Error(data?.message || 'Não foi possível concluir a solicitação.'); return data; }
  ...
  async function handleLogout() { try { await fetch(`${apiBase}/auth/logout`, { method: 'POST', credentials: 'include' }); } catch { /* limpa a sessão local mesmo se a chamada falhar */ } localStorage.removeItem('nfse_user'); router.replace('/login'); }
