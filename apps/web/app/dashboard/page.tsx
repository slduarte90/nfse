...
function PermissionMatrix({ value, onChange, disabled = false }: { value: CompanyPermission[]; onChange: (next: CompanyPermission[]) => void; disabled?: boolean }) {
  const [openModules, setOpenModules] = useState<Set<string>>(() => new Set([COMPANY_PERMISSION_MODULES[0]?.title].filter(Boolean) as string[]));
...
export default function DashboardPage() {
  ...
  return (
    <main className="companies-page">
      ...
      <section className="companies-grid" aria-label="Lista de empresas">{isLoading ? <p className="companies-empty">Carregando empresas...</p> : null}{!isLoading && visibleCompanies.length === 0 ? <p className="companies-empty">Nenhuma empresa encontrada.</p> : null}{visibleCompanies.map((company) => <article className={`company-card ${company.isActive === false ? 'is-inactive' : ''}`} key={company.id}><div className="company-card__menu-wrap"><button className="company-card__menu-button" type="button" aria-label="Abrir ações da empresa" onClick={() => setOpenMenuCompanyId(openMenuCompanyId === company.id ? null : company.id)}>...</button>{openMenuCompanyId === company.id ? <div className="company-card__dropdown">{isSystemAdmin ? <button type="button" onClick={() => openEditCompany(company)}>Editar Empresa</button> : null}<button type="button" onClick={() => void openUsersModal(company)}>Ver usuários</button>{isSystemAdmin && company.isActive !== false ? <button type="button" onClick={() => void inactivateCompany(company)}>Inativar empresa</button> : null}{isSystemAdmin && company.isActive === false ? <button className="is-danger" type="button" onClick={() => void removeInactiveCompany(company)}>Excluir empresa</button> : null}</div> : null}</div><button className="company-card__content" type="button" onClick={() => router.push(`/empresas/${company.id}`)}><h2 title={company.legalName}>{company.legalName}</h2><p>{formatCnpj(company.cnpj)}</p><div className="company-card__meta"><span>{company.city}/{company.state}</span><span>{company.isActive === false ? 'Inativa' : company.taxRegime}</span></div></button></article>)}</section>
