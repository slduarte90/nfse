...
type CompanyAccessUser = { id: string; name: string; email: string; role: CompanyRole; status: CompanyUserStatus; isActive: boolean; accountRole: AccountRole; canDelete?: boolean; linkedInvoices?: number };
type UserCompanyAccess = { id: string; legalName: string; tradeName?: string | null; cnpj: string; city: string; state: string; isActive: boolean; role: CompanyRole; status: CompanyUserStatus };
...
type EditUserForm = { name: string; email: string; accountRole: AccountRole; role: 'OPERATOR' | 'VIEWER' | 'ADMIN'; companyIds: string[] };
type CreateCompanyForm = {
  document: string; legalName: string; tradeName: string; municipalRegistration: string; city: string; state: string; country: string; zipCode: string; address: string; number: string; complement: string; neighborhood: string; email: string; phone: string; mobile: string; contactPerson: string; website: string; registrationStatus: string; mainActivity: string; legalNature: string; taxRegime: string; serviceCodeDefault: string;
};
...
const emptyEditUserForm: EditUserForm = { name: '', email: '', accountRole: 'USER', role: 'OPERATOR', companyIds: [] };
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    void loadCompanies('', { updateAll: true });
  }, [router]);
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => void loadAdminUsers(userSearch, userStatusFilter), 250);
    return () => window.clearTimeout(timer);
  }, [userSearch, userStatusFilter, activeModal]);

  useEffect(() => {
    if (!selectedAdminUser) { setEditUserForm(emptyEditUserForm); return; }
    setEditUserForm({ name: selectedAdminUser.name, email: selectedAdminUser.email, accountRole: selectedAdminUser.accountRole, role: getPrimaryCompanyRole(selectedAdminUser.companies), companyIds: selectedAdminUser.companies.map((company) => company.id) });
  }, [selectedAdminUserId, adminUsers]);

  async function requestApi(path: string, options: RequestInit = {}) {
    const token = localStorage.getItem('nfse_access_token');
    if (!token) { router.replace('/login'); throw new Error('Sessão expirada.'); }
    const response = await fetch(`http://localhost:3333${path}`, { ...options, headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}`, ...(options.headers || {}) } });
  ...
  async function loadCompanies(searchTerm = '', options: { updateAll?: boolean } = {}) {
  ...
  async function loadAllCompaniesForInvite() {
    ...
    catch (err) { const message = err instanceof Error ? err.message : 'Não foi possível carregar todas as empresas.'; activeModal ? setModalErrorMessage(message) : setError(message); }
  }

  async function loadAdminUsers(term = '', status = userStatusFilter) {
    ...
    try {
      ...
      const data = (await requestApi(`/users${query}`)) as AdminUser[];
      setAdminUsers(data);
      setSelectedAdminUserId((current) => (current && data.some((item) => item.id === current) ? current : data[0]?.id || null));
    } catch (err) { setModalErrorMessage(err instanceof Error ? err.message : 'Não foi possível carregar usuários.'); }
    finally { setIsAdminUsersLoading(false); }
  }

  async function handleLookupCnpj() {
    const cnpj = onlyDigits(companyForm.document);
    setLookupError(''); clearModalMessages();
    ...
    try {
      ...
      setModalSuccessMessage('Dados cadastrais localizados. Confira as informações antes de cadastrar.');
    } catch (err) { setLookupError(err instanceof Error ? err.message : 'Não foi possível consultar o CNPJ agora.'); }
  ...
  async function handleLookupCep() {
    ...
    try {
      ...
      setModalSuccessMessage('Endereço localizado pelo CEP. Confira as informações antes de cadastrar.');
  ...
  async function handleCreateCompany(event: FormEvent<HTMLFormElement>) {
    event.preventDefault(); clearModalMessages(); setLookupError('');
    ...
    if (clientType !== 'PJ') { setModalErrorMessage('Cadastro definitivo de Pessoa Física e Exterior será conectado ao backend na próxima etapa.'); return; }
    if (!companyForm.legalName || !companyForm.city || !companyForm.state) { setModalErrorMessage('Busque o CNPJ ou preencha razão social, cidade e UF antes de cadastrar.'); return; }
    try {
      ...
      setModalSuccessMessage('Empresa cadastrada com sucesso.');
      await loadCompanies(search);
      await loadAllCompaniesForInvite();
    } catch (err) { setModalErrorMessage(err instanceof Error ? err.message : 'Erro ao criar empresa.'); }
  }

  async function handleInviteUser(event: FormEvent<HTMLFormElement>) {
    event.preventDefault(); clearModalMessages();
    if (inviteForm.companyIds.length === 0) { setModalErrorMessage('Selecione ao menos uma empresa para o usuário acessar.'); return; }
    try {
      ...
      setModalSuccessMessage(data.message || 'Convite registrado com sucesso.');
      setInviteForm(emptyInviteForm);
    } catch (err) { setModalErrorMessage(err instanceof Error ? err.message : 'Erro ao registrar convite.'); }
  }
  ...
  function toggleEditCompany(companyId: string) { setEditUserForm((current) => ({ ...current, companyIds: current.companyIds.includes(companyId) ? current.companyIds.filter((id) => id !== companyId) : [...current.companyIds, companyId] })); }

  async function openUsersModal(company: Company) {
    setSelectedCompany(company); setActiveModal('companyUsers'); setOpenMenuCompanyId(null); setIsUsersLoading(true); clearModalMessages();
  ...
  async function unlinkCompanyUser(targetUser: CompanyAccessUser) {
    ...
    try {
      ...
      setModalSuccessMessage(data.message || 'Usuário desvinculado.');
      await openUsersModal(selectedCompany);
      setModalSuccessMessage(data.message || 'Usuário desvinculado.');
    } catch (err) { setModalErrorMessage(err instanceof Error ? err.message : 'Não foi possível desvincular usuário.'); }
  }

  async function saveAdminUser() {
    if (!selectedAdminUser) return;
    clearModalMessages();
    if (!editUserForm.name.trim() || !editUserForm.email.trim()) { setModalErrorMessage('Nome e e-mail são obrigatórios.'); return; }
    try {
      const data = await requestApi(`/users/${selectedAdminUser.id}`, { method: 'PATCH', body: JSON.stringify(editUserForm) });
      setModalSuccessMessage(data.message || 'Usuário atualizado.');
      await loadAdminUsers(userSearch, userStatusFilter);
      await loadAllCompaniesForInvite();
  ...
  async function activateAdminUser() {
    ...
    clearModalMessages();
    try {
      const data = await requestApi(`/users/${selectedAdminUser.id}/activate`, { method: 'PATCH' });
      setModalSuccessMessage(data.message || 'Usuário ativado.');
      await loadAdminUsers(userSearch, userStatusFilter);
    } catch (err) { setModalErrorMessage(err instanceof Error ? err.message : 'Não foi possível ativar usuário.'); }
  }
  ...
  async function deactivateAdminUser() {
    ...
    clearModalMessages();
    try {
      const data = await requestApi(`/users/${selectedAdminUser.id}/deactivate`, { method: 'PATCH' });
      setModalSuccessMessage(data.message || 'Usuário inativado.');
      await loadAdminUsers(userSearch, userStatusFilter);
    } catch (err) { setModalErrorMessage(err instanceof Error ? err.message : 'Não foi possível inativar usuário.'); }
  }

  function handleLogout() { localStorage.removeItem('nfse_access_token'); localStorage.removeItem('nfse_user'); router.replace('/login'); }

  async function openInviteForm() { setActiveModal('invite'); setError(''); setSuccess(''); clearModalMessages(); await loadAllCompaniesForInvite(); }
  function openCreateForm() { setActiveModal('create'); setActiveClientTab('contact'); setLookupError(''); setCepError(''); setError(''); setSuccess(''); clearModalMessages(); }
  async function openAdminUsersModal() { setActiveModal('users'); setUserSearch(''); setUserStatusFilter('ALL'); setError(''); setSuccess(''); clearModalMessages(); await loadAllCompaniesForInvite(); await loadAdminUsers('', 'ALL'); }
  function closeModal() { setActiveModal(null); clearModalMessages(); }
  ...
  const modalMessages = (
    <>
      {modalError ? <p className="companies-alert companies-alert--error">{modalError}</p> : null}
      {modalSuccess ? <p className="companies-alert companies-alert--success">{modalSuccess}</p> : null}
    ...
    <main className="companies-page">
      <header className="companies-header">
        ...
        <div className="companies-actions">
          {isSystemAdmin ? <><button className="companies-button companies-button--light" type="button" onClick={() => void openAdminUsersModal()}>👥 Usuários</button><button className="companies-button companies-button--light" type="button" onClick={() => void openInviteForm()}>✉ Convidar usuários</button><button className="companies-button companies-button--primary" type="button" onClick={openCreateForm}>+ Criar empresa</button></> : null}
      ...
      {!activeModal && error ? <p className="companies-alert companies-alert--error">{error}</p> : null}
      {!activeModal && success ? <p className="companies-alert companies-alert--success">{success}</p> : null}
