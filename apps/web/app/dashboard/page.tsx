...
function statusLabel(status: string) {
  return ({ ACTIVE: 'Ativo', BLOCKED: 'Bloqueado', DISABLED: 'Desativado' } as Record<string, string>)[status] || status;
}

function getPrimaryCompanyRole(companies: UserCompanyAccess[]): EditUserForm['role'] {
  const role = companies.find((item) => item.role !== 'OWNER')?.role || companies[0]?.role || 'OPERATOR';
  return role === 'VIEWER' || role === 'ADMIN' ? role : 'OPERATOR';
}
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    setEditUserForm({
      ...
      accountRole: selectedAdminUser.accountRole,
      role: getPrimaryCompanyRole(selectedAdminUser.companies),
      companyIds: selectedAdminUser.companies.map((company) => company.id),
    });
  ...
  async function loadAllCompaniesForInvite() {
    try {
      setAllCompanies((await requestApi('/companies')) as Company[]);
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Não foi possível carregar todas as empresas.');
    }
  }
  ...
  async function handleCreateCompany(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      await requestApi('/companies', {
        method: 'POST',
        body: JSON.stringify({ ...companyForm, cnpj: onlyDigits(companyForm.document), zipCode: onlyDigits(companyForm.zipCode) }),
      });
  ...
  async function handleInviteUser(event: FormEvent<HTMLFormElement>) {
    ...
    setSuccess('');
    setInviteLink('');
    if (inviteForm.companyIds.length === 0) {
      setError('Selecione ao menos uma empresa para o usuário acessar.');
      return;
    }
    try {
      ...
      setError(err instanceof Error ? err.message : 'Erro ao registrar convite.');
    }
  }
  ...
  function toggleEditCompany(companyId: string) {
    setEditUserForm((current) => ({ ...current, companyIds: current.companyIds.includes(companyId) ? current.companyIds.filter((id) => id !== companyId) : [...current.companyIds, companyId] }));
  }

  async function openUsersModal(company: Company) {
    setSelectedCompany(company);
    setActiveModal('companyUsers');
    setOpenMenuCompanyId(null);
    setIsUsersLoading(true);
    setError('');
    try {
      setCompanyUsers((await requestApi(`/companies/${company.id}/users`)) as CompanyAccessUser[]);
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Não foi possível carregar usuários.');
    } finally {
  ...
  async function unlinkCompanyUser(targetUser: CompanyAccessUser) {
    ...
    try {
      ...
      setSuccess(data.message || 'Usuário desvinculado.');
      await openUsersModal(selectedCompany);
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Não foi possível desvincular usuário.');
    }
  }
  ...
  async function saveAdminUser() {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'Não foi possível salvar usuário.');
    }
  }
  ...
  async function activateAdminUser() {
    ...
    try {
      ...
      setSuccess(data.message || 'Usuário ativado.');
      await loadAdminUsers(userSearch, userStatusFilter);
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Não foi possível ativar usuário.');
    }
  }
  ...
  async function deactivateAdminUser() {
    ...
    try {
      ...
      setSuccess(data.message || 'Usuário inativado.');
      await loadAdminUsers(userSearch, userStatusFilter);
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Não foi possível inativar usuário.');
    }
  }
    ...
    <main className="companies-page">
      <header className="companies-header">
        ...
        <div className="companies-actions">
          {isSystemAdmin ? (
            <>
              <button className="companies-button companies-button--light" type="button" onClick={() => void openAdminUsersModal()}>👥 Usuários</button>
      ...
      {activeModal ? (
        <div className="modal-backdrop" role="presentation">
          <section className={`modal-card ${activeModal === 'create' || activeModal === 'users' ? 'modal-card--wide' : ''}`} role="dialog" aria-modal="true">
            <button className="companies-close modal-close" type="button" onClick={closeModal}>×</button>

            {activeModal === 'users' ? (
              <>
                <div className="modal-heading"><h2>Usuários</h2><p>Edite dados, categoria, perfil e empresas vinculadas ao usuário.</p></div>
                <div className="admin-users-layout">
                  <div className="admin-users-list">
                    <div className="admin-users-filters">
                      <input className="admin-users-search" value={userSearch} onChange={(event) => setUserSearch(event.target.value)} placeholder="Buscar por nome ou e-mail..." />
                    ...
                    {isAdminUsersLoading ? <p className="companies-empty">Carregando usuários...</p> : null}
                    {!isAdminUsersLoading && adminUsers.length === 0 ? <p className="companies-empty">Nenhum usuário encontrado.</p> : null}
                    {adminUsers.map((item) => (
                      <button key={item.id} className={`admin-user-card ${selectedAdminUserId === item.id ? 'is-active' : ''}`} type="button" onClick={() => setSelectedAdminUserId(item.id)}>
                        <strong>{item.name}</strong>
                        <span>{item.email}</span>
                        <small>{item.isActive ? 'Ativo' : 'Inativo'} · {item.accountRole === 'ADMIN' ? 'Administrador do sistema' : 'Usuário da empresa'} · {item.companiesCount} empresa(s)</small>
                      </button>
                    ))}
                  </div>
                    ...
                    {selectedAdminUser ? (
                      <>
                        <div className="admin-user-detail__header">
                          ...
                          <span>{selectedAdminUser.accountRole === 'ADMIN' ? 'Administrador do sistema' : 'Usuário da empresa'}</span>
                        </div>
                        <div className="companies-form admin-user-edit">
                          <label>Nome<input value={editUserForm.name} onChange={(event) => setEditUserForm((current) => ({ ...current, name: event.target.value }))} /></label>
                          <label>E-mail<input type="email" value={editUserForm.email} onChange={(event) => setEditUserForm((current) => ({ ...current, email: event.target.value }))} /></label>
                          <label>Categoria<select value={editUserForm.accountRole} onChange={(event) => setEditUserForm((current) => ({ ...current, accountRole: event.target.value as AccountRole }))}><option value="USER">Usuário da empresa</option><option value="ADMIN">Administrador do sistema</option></select></label>
                          <label>Perfil por empresa<select value={editUserForm.role} onChange={(event) => setEditUserForm((current) => ({ ...current, role: event.target.value as EditUserForm['role'] }))}><option value="OPERATOR">Operador</option><option value="VIEWER">Visualizador</option><option value="ADMIN">Administrador da empresa</option></select></label>
                        </div>
                        <fieldset className="companies-checkboxes user-company-checkboxes">
                          <legend>Empresas selecionadas</legend>
                          {inviteCompanies.map((company) => (
                            <label key={company.id} className="company-checkbox company-checkbox--compact"><input type="checkbox" checked={editUserForm.companyIds.includes(company.id)} onChange={() => toggleEditCompany(company.id)} /><span title={company.legalName}>{company.legalName}</span></label>
                          ))}
                        </fieldset>
                        <div className="admin-user-actions">
                          <button className="companies-button companies-button--primary" type="button" onClick={() => void saveAdminUser()}>Salvar alterações</button>
                          {selectedAdminUser.isActive ? (
                            <button className="companies-button companies-button--danger" type="button" onClick={() => void deactivateAdminUser()}>Inativar</button>
                          ) : (
                            <button className="companies-button companies-button--primary" type="button" onClick={() => void activateAdminUser()}>Ativar</button>
                          )}
                        </div>
                      </>
                    ) : <p className="companies-empty">Selecione um usuário.</p>}
                  </div>
                </div>
              </>
            ) : null}
